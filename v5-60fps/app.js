...
/* The take was cut slow for a scroll, so it is sped up — but in the file, not
   with playbackRate. The original is 24fps; at 2.5x that is exactly 60fps, so
   every frame is kept and each one lands on exactly one refresh of a 60Hz
   screen. An off-rate <video> cannot do that: 24fps at 2x is 48 frames on 60
   refreshes, held 2,1,1,1,2,1,1,1… which is the judder you were feeling.

   Every mark below is written in the ORIGINAL take's seconds, because that is
   what the footage was measured in — SPEED converts them once, at load. */
const SPEED = 2.5;
BEATS.forEach(b => { b.t /= SPEED; });
for (let i = 0; i < EXITS.length; i++) EXITS[i] /= SPEED;
...
const REWIND  = 5;                                 // scroll-up runs the footage back this fast
...
function drive() {
  ...
  if (gap > 0) {
    ...
    plate.play().catch(() => {});   // 1.0 rate, native decode: the browser's fast path
    const fwd = () => {
